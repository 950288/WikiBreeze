  ...
  <div class="Input">
    ...
    <div class="body">
      ...
      <div class="confirm" v-if="uploadEnable">
        <button class="button is-primary" @click="upload()">
          {{ "upload" }}
        </button>
...
function confirm() {
  ...
  if (props.recall) props.recall.value = url.value;
}
function destroy() {
...
onMounted(() => {
  upload.value = async () => {
    if (!file.value) {
      notifyStore.notify(1500, "uploads", "No file selected !", "info", null);
      return;
    }
    ...
    var renamedFile = new File(
      [file.value[0]],
      `${hash}.${file.value[0].name.split(".").pop()}`,
      { type: file.value[0].type },
    );
    fd.append("file", renamedFile);
    fd.append(
      "directory",
      "WikiBreezeUpload/" + history.state.index + "/" + history.state.content,
    );
    ...
    notifyStore.notify(
      ...
      "info",
      new Promise((resolve) => {
        let { data: fileURL, error: uploadError } = useFetch(
